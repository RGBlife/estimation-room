...
export const useRoomStore = create<RoomState>((set, get) => {
  const connection = new RoomConnection(message => {
    switch (message.type) {
      ...
      case 'drivers': set({ drivers: message.drivers }); break;
      case 'driver': set(s => {
        const drivers = { ...s.drivers };
        if (message.driver) drivers[message.uid] = message.driver;
        else delete drivers[message.uid];
        return { drivers };
      }); break;
      ...
      case 'closed':
        connection.forgetRoom(); driving = false;
