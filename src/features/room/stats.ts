import { CARD_VALUES } from '../avatar/avatar.ts';
...
export function computeStats(participants: Record<string, Participant>): Stats {
...
export function computeDistribution(participants: Record<string, Participant>): DistributionGroup[] {
  ...
  return CARD_VALUES.filter((value) => groups[value]).map((value) => {
