...
- **Early reveal:** any participant can reveal after at least one vote, including with Enter. Players without a vote keep their cards or custom input and can submit into the revealed results. Their dock offers expandable current results until they vote; the table keeps a live vote count. Number shortcuts remain available, while Enter does not advance the round for a player still choosing. The seven-client emulator check verifies a late vote updates the count and average without hiding results. Existing Firebase rules already permit these participant vote updates.
- **Nudge:** a bell on each waiting player’s card offers a direct nudge; it is hidden for yourself, observers and submitted votes. A nudge produces a short shake and a text reminder; reduced-motion users receive text only. The UI limits sending and receiving to one nudge per ten seconds. Nudges use the existing transient throw transport, with an additional allowed event type in `database.rules.json`.
...
Before nudges or the six new observer weapons are used on a deployed site, the matching Realtime Database rules must be deployed. Everyone in a multiplayer validation session should refresh to the same build; older clients still contain the previous table-reset behaviour. `main` and the deployed rules were left unchanged.

## Observer throws and in-room avatars

Observers have peanut, tomato, spec-document plane, rubber chicken, cheese and stale pastry. Voters retain their existing tray. Both roles use the same throw transport and rendered avatar targets. The chicken adds a short synthesised squeak after impact; browsers need a prior user interaction to enable audio. The tray includes a per-device mute preference.

The header avatar opens the existing customiser in a modal dialog with draft changes, Cancel and Save. Saving updates only the caller's avatar fields, converts legacy URL avatars to the current format, and saves the local profile after Firebase acknowledges the write. Existing Firestore participant-edit rules cover this change. The dialog keeps keyboard focus inside and restores it on close.

The observer multiplayer test joins through the UI, sends each of the six weapons to another browser, saves a shuffled avatar and checks its remote appearance while preserving the observer role and vote. The responsive test covers the tray, editor, visible save action and restored focus on desktop, tablet and phone.
